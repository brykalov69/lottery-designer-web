...
export default function TicketGeneratorPanel({ isPro }: { isPro: boolean }) {
    ...
    const loadTickets = async () => {
        setLoading(true);
        try {
            const res = await fetch(`http://localhost:8000/ai_tickets?is_pro=${isPro}`);
            const json = await res.json();
            setData(json);
        } catch (e) {
            console.error(e);
            setData({ mode: "error", tickets: {}, error: "Failed to load tickets." });
        }
        setLoading(false);
    };
    ...
    return (
        <div>
            <h4 style={{ color: "#eee", marginBottom: 15 }}>AI Ticket Generator</h4>
